...
Menampilkan Main Menu yang terdiri dari 5 menu: (menggunakan looping `while` agar setiap selesai dengan menu yg dipilih akan selalu kembali ke Main Menu, kecuali pilih no 5. Exit (`break` looping)
  ...
  2. Stock Buku (`function stock_buku` dan looping dengan `while`)
     Di menu ini, user dapat memilih:
     (1) Menambah stock buku (menambah jumlah stock buku yang sudah ada)-> menggunakan `function tambah_stock_buku` -> jika           salah menginput index juga akan kembali ke user input index awal karena menggunakan `except indexerror`.
     (2) Mengurang stock buku (mengurangi jumlah stock buku yang sudah ada) -> menggunakan `function kurang_stock_buku` ->            jika salah menginput index juga akan kembali ke user input index awal karena menggunakan `except indexerror`.
     (3) Menambah List Baru (menambah list buku baru) -> menggunakan `function tambah_list_baru` -> jika kode yang                    dimasukkan sama dengan kode yang sudah ada, maka user akan kembali menginput dari awal, agar kodenya tidak sama              (karena buku baru) menggunakan function `any (dictionary.get('kode') == kodeBuku`
